Class(function RacerDevice() {
    ...
    this.mobile = (!window.location.hash.strpos('table') && Config.FORCE_MOBILE) || Device.mobile;
    ...
    this.chrome = Mobile.browser == 'Chrome' || Device.browser.chrome || Device.detect('crios');
    ...
    this.chrome_version = (function() {
        ...
        if (_this.chrome && _this.android && version) return Number(version.split('.')[0]);
        if (_this.chrome && _this.iOS && version) return Number(version.split('.')[0]);
    ...
    function init() {
        _this.doc = (Device.browser.ie || Device.browser.firefox) ? document.documentElement : document.body;
    ...
    function refreshPage() {
        var $overlay = $('.overlay');
        $overlay.size('100%').bg(Config.BG).css({ top: 0, left: 0, opacity: 0, display: 'block' }).setZ(100000);
    ...
    function resizeHandler() {
        ...
        _this.isKeyboard = ((w == _this.width && h !== _this.height) || (RacerDevice.android && h < 250)) || (w == _this.width && h == _this.height && h != _this.previousHeight);
