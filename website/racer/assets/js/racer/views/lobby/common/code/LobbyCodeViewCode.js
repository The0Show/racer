Class(function LobbyCodeViewCode(_config) {
    ...
    function initHTML() {
        ...
        $overlay = $('.overlay');
        $overlay.size('100%').bg(Config.BG).setZ(1000);
        $overlay.invisible();
    ...
    function initCode() {
        ...
        for (var i = 0; i < _config.length; i++) {
            var single = _this.initClass(LobbyCodeViewCodeSingle, { border: _config.border, size: 10, index: Number(i), set: '', name: _config.name, input: _config.input });
    ...
    this.update = function(e) {
        ...
        for (var i = 0; i < _singles.length; i++) {
            var code = input[i] || '';
            var isEnd = (i == _singles.length-1 && i == input.length-1) ? true : false;
    ...
    this.animateOut = function(callback) {
        ...
        $overlay.visible().css({ opacity: 0 }).tween({ opacity: 1 }, 300, 'easeOutSine', 100, function(){
