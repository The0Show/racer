Class(function LobbyPlayerViewPlayer(_index) {
    Inherit(this, View);
    var _this = this;
    var $this, $overlay, $wrapper, $border, $bg, $result, $resultBehind, $place, $time, $remove;
    ...
    function initHTML() {
        $this = _this.element;
        $this.size('100%', _height).css({ webkitBackfaceVisibility: 'hidden' }).setZ(1);
        if (!Global.RESULTS) $this.hide();
    }
    
    function initWrapper() {  
        $wrapper = $('.wrapper');
        $wrapper.size('100%').css({ borderRadius: 2, top: 0, left: 0 }).bg('#2a2a2a').setZ(2);
        $wrapper.hide();
    ...
    function initName() {
        _name = _this.initClass(LobbyCodeView, { border: false, length: 3, text: false, input: RacerDevice.mobile, keepInput: true, name: true });
        _name.nameSet('???');
    ...
    function initTime() {
        ...
        $time.css({ textAlign: 'right', fontWeight: 'bold', overflow: 'visible', right: 10, width: '100%', opacity: 0 }).setZ(11);
        $wrapper.inner.addChild($time);
    }
    
    function initRemove() {
        $remove = $('.remove');
        $remove.size('100%').css({ overflow: 'hidden', borderRadius: 2, display: 'block', boxShadow: '0 0 3px #000' }).setZ(10000);
        $remove.hide();
        ...
        $remove.close = $('.close');
        $remove.close.size(160,160).bg(Config.PATH+'assets/images/common/close.png');
        $remove.close.setZ(10001);
    ...
    function resizeHandler() {
        ...
        if (_this.data && _this.data.me) {
            ...
            $border.css({ boxShadow: '0 0 '+$border.setWidth*4+'px '+color, border: $border.setWidth+'px solid rgba(255,255,255,0.15)' });    
        }
        
        _name.resize(_height);
        _name.css({ top: 0, left: Global.RESULTS ? _height*1.5 : $border.setWidth*2.5 });
        
        if (_you) {
            _you.css({ left: (Global.RESULTS ? _height*3.3 : _height*2)+$border.setWidth*4  });
            _you.scale = _height < 60 ? _height/60 : 1;
            _you.element.transform({ scale: _you.scale });
        }
        
        if ($result) $result.size(_height*0.8, _height*0.528).css({ left: $border.setWidth*3, top: _height*0.5-_height*0.25 });
        if ($resultBehind && $resultBehind.size) $resultBehind.size(_height*0.8, _height*0.528).css({ left: $border.setWidth*3, top: _height*0.5-_height*0.264 });
        
        var fontSize = _height*0.35;
        if (fontSize < 13) fontSize = 13;
        if ($place) $place.css({ left: _height*3.3+$border.setWidth*4, fontSize: fontSize, top: _height*0.5-fontSize*0.42, letterSpacing: fontSize*0.065+'px' });
        if ($time) $time.css({ right: _height*0.45-2, fontSize: fontSize*0.88, top: _height*0.5-fontSize*0.36, letterSpacing: fontSize*0.065+'px' });
        if ($remove.inner) $remove.inner.css({ left: _height*1.2, fontSize: fontSize*0.88, top: _height*0.5-fontSize*0.37, letterSpacing: fontSize*0.06+'px' });
        if ($remove.close) $remove.close.size(_height*0.6,_height*0.6).css({ top: _height*0.22, left: _height*0.22 })
    }
    
    function setStyle(noAnim) {
        var gradient = RacerUtil.gradient(_this.data.gradient[0], _this.data.gradient[1], _this.data.gradient[2]);
        if (_this.data.me) {
        ...
        if (_this.data.me) {
            ...
            if (noAnim) {
                ...
                if ($overlay) $overlay.tween({ opacity: 0 }, 300, 'easeOutSine', $overlay.hide);    
            } else {
            ...
            if (noAnim) {
                ...
                if ($overlay) $overlay.stopTween().show().tween({ opacity: 0.4 }, 300, 'easeOutSine'); 
            } else {
        ...
        if (Global.RESULTS) {
            ...
            if (_this.data.placement) {
                $time.text(_this.data.placement.lap_time).css({ fontWeight: 'bold' }).show().clearAlpha();
                if ($result) {
                    if ($result.place !== _this.data.placement.place) $result.bg(Config.PATH+'assets/images/common/places/'+_this.data.placement.place+'.png');
                    $result.place = _this.data.placement.place;
                }
                if ($resultBehind && $resultBehind.bg) $resultBehind.bg(Config.PATH+'assets/images/common/places/'+_this.data.placement.place+'.png').css({ opacity: 0.9-(_this.data.placement.place*0.15) });
            } else {
                $time.hide().text('');
                if ($result) {
                    if ($result.place !== -1) $result.bg(Config.PATH+'assets/images/common/places/empty.png');
                    $result.place = -1;
                }
                if ($resultBehind && $resultBehind.bg) $resultBehind.bg(Config.PATH+'assets/images/common/places/empty.png');
            }
        } else {
            ...
            if (_this.data.placement) {
                $time.text(_this.data.placement.lap_time).css({ fontWeight: 'bold' }).show().clearAlpha();
                $place.text(RacerUtil.formatPlaceWord(_this.data.placement.place)).css({ fontWeight: 'bold' }).show().clearAlpha();
            } else {
                $time.hide().text('');
                $place.hide().text('');
            }
    ...
    function showRemove(left) {
        ...
        _this.removing = true;
        $remove.show();
        $remove.inner.transform({ x: left ? 10 : -10 }).css({ opacity: 0 }).tween({ opacity: 1, x: 0 }, 300, 'easeOutCubic', left ? 200 : 0, $remove.inner.clearTransform);
        $remove.close.transform({ scale: 0.7 }).css({ opacity: 0 }).tween({ scale: 1, opacity: 1 }, 300, 'easeOutCubic', left ? 200 : 0, $remove.close.clearTransform);
        $remove.back.transform({ x: left ? _width : -_width }).tween({ x: 0 }, 400, 'easeOutCubic', $remove.back.clearTransform);
    }
    ...
    this.fadeIn = function() {
        $wrapper.inner.show().css({ opacity: 0 }).tween({ opacity: 1 }, 200, 'easeOutSine', function(){
    ...
    this.setText = function(time, position) {
        _this.isSet = true;
        if ($time) $time.text(time).css({ fontWeight: 'bold' }).tween({ opacity: 1 }, 300, 'easeOutSine');
        if ($place) $place.text(position).css({ fontWeight: 'bold' }).tween({ opacity: 1 }, 300, 'easeOutSine');
    }
    
    this.updateTime = function(time) {
        if (!_time) $time.css({ fontWeight: 'bold' }).tween({ opacity: 0.7 }, 300, 'easeOutSine');
        _time = time;
        $time.show().text(time);
    }
    
    this.updatePosition = function(position) {
        if (!_position) $place.css({ fontWeight: 'normal' }).tween({ opacity: 0.7 }, 300, 'easeOutSine');
        _position = position;
    ...
    this.animateIn = function() {
        ...
        if (_this.data && (_this.data.placement || (Global.RESULTS && _this.data.name))) {
            $wrapper.transform({ x: _width/5 }).css({ opacity: 0 }).tween({ opacity: 1, x: 0 }, 400, 'easeOutCubic');
        } else {
            $wrapper.transform({ y: _height/3 }).css({ opacity: 0 }).tween({ opacity: 1, y: 0 }, 400, 'easeOutCubic');
        }
    ...
    this.animateOut = function() {
        _this.visible = false;
        if (RacerDevice.animate) $wrapper.tween({ y: -_height/3, opacity: 0 }, 300, 'easeInCubic', $this.hide);
    }
});
