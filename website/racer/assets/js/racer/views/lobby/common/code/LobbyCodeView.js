Class(function LobbyCodeView(_config) {
    ...
    function initButtons() {
        $buttons = $('.buttons');
        $buttons.css({ width: 230, height: 50, bottom: -70, left: '50%', marginLeft: -115 }).setZ(1000);
        $this.addChild($buttons);
        
        _join = _this.initClass(HorizontalButtonView, { text: 'JOIN', width: 125, height: 50 });
        ...
        if (RacerDevice.mobile && !Global.TABLE) {
            ...
            $instruction.css({ fontWeight: 'bold', width: '100%', whiteSpace: 'nowrap', letterSpacing: '1px', bottom: -150 });
    ...
    function resizeHandler() {
        _this.width = Math.round(_size*_config.length+_gap*(_config.length-1));
        _this.height = _size;
        ...
        if ($buttons && $buttons.css) $buttons.css({ height: height, width: width, bottom: -height*1.25, left: '50%', marginLeft: -width/2 });
        if (_join) _join.resize(Global.TABLE ? width : width*0.6, height, height*0.3);
        if (_join && _join.css) _join.css({ left: Global.TABLE ? -width*0.01 : width*0.4, marginLeft: 0 });
        if (_exit) _exit.resize(width*0.35, height, height*0.32);
        if (_exit && _exit.css) _exit.css({ left: 0, marginLeft: 0 });
        if ($instruction && $instruction.css) $instruction.css({ fontSize: height*0.2, bottom: -height*2.3, letterSpacing: height*0.02+'px', lineHeight: height*0.28 });
    }
    ...
    function inputUpdate(e) {
        if (e) {
            ...
            if (_input.code.length == _config.length) {
                ...
                if (_config.name && _input && _input.code && _input.code.length > 0) autoSumbit(3000);
    ...
    function formSubmit(e) {
        if ((_input && (_input.code && _input.code.length == _config.length)) || _config.name) {
    ...
    function doneClick(e) {
        if (!_this.clicked) {
            if (RacerDevice.mobile) GATracker.trackEvent("clickable_link", "enter_code", "Join");
            else GATracker.trackEvent("clickable_link", "desktop_live", "Join");
            if (_timeout) clearTimeout(_timeout);
            _this.clicked = true;
            if (_input && _input.blur) _input.blur();
            if (!_config.text && $this && $this.tween) $this.tween({ opacity: 1 }, 150, 'easeOutSine');
            if (_input && (_input.code || _config.name)) _this.events.fire(HydraEvents.COMPLETE, { code: _input.code.toUpperCase() });   
        }
    }
    
    function exitClick() {
        if (RacerDevice.mobile) GATracker.trackEvent("clickable_link", "enter_code", "Exit");
    ...
    this.set = function(syncCode, isHost, noAnim) {
        ...
        if ($instruction && $instruction.tween) $instruction.tween({ opacity: 0 }, 300, 'easeOutSine');
    ...
    this.animateIn = function(syncCode, noAnim) {
        ...
        if (_config.input && !syncCode) {
            ...
            if ($instruction && $instruction.visible) $instruction.visible().css({ opacity: 0 }).transform({ y: 10 }).tween({ opacity: 1, y: 0 }, 300, 'easeOutCubic', 700, function(){ $instruction.clearTransform().clearAlpha(); });
