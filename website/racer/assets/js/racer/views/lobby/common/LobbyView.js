Class(function LobbyView(_data) {
    ...
    function initText() {
        ...
        $text.text('FRIENDS CAN JOIN AT <strong>G.CO/RACER</strong><br/>BY ENTERING THIS RACE CODE:');
        $text.invisible();
        $this.addChild($text);
    ...
    function initResults() {
        ...
        if (RacerDevice.mobile) {
            ...
            $text.text('NEW RACERS CAN JOIN AT <strong>G.CO/RACER</strong>');
        } else {
            $results.text('THE RACERS');
            $text.text('CHALLENGERS CAN JOIN AT <strong>G.CO/RACER</strong>');    
        }
    }
    ...
    this.setAsResult = function() {
        ...
        if (RacerDevice.mobile) {
            ...
            $results.text('CHALLENGERS CAN JOIN AT <strong>G.CO/RACER</strong>');
        ...
        if (RacerDevice.mobile) {
            ...
            if (e.players.length > 1) {
                if (e.players.length == 5) {
                    ...
                    if (!_this.watcher && !Global.RESULTS) $text.text('FRIENDS CAN JOIN AT <strong>G.CO/RACER</strong><br/>BY ENTERING THIS RACE CODE:');
                }
            } else {
                if (_setCode && !_this.watcher && !Global.RESULTS) $text.text('FRIENDS CAN JOIN AT <strong>G.CO/RACER</strong><br/>BY ENTERING THIS RACE CODE:');
            }
        }
    }
