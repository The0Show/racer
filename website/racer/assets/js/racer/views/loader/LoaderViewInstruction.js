Class(function LoaderViewInstruction(_config) {
    ...
    function initHTML() {
        $this = _this.element;
        $this.css({ width: '100%', height: 20, top: '45%', left: 0, overflow: 'hidden' });
        $this.invisible();
    ...
    function positionText() {
        ...
        $text.css({ fontSize: fontSize, paddingLeft: 7, letterSpacing: fontSize*0.05+'px' });
        $text.width = $text.div.offsetWidth;
        $text.css({ textAlign: 'left', width: $text.width, left: '50%', marginLeft: -fontSize*7 });
        $line.css({ top: fontSize*0.4 });
        
        $this.css({ marginTop: fontSize*1.3*_config.index-(fontSize*3.9), height: fontSize });
    }

    //*** Public Methods
    this.animateIn = function() {
        $this.visible();
        $line.transform({ x: -window.innerWidth }).tween({ x: 0 }, 400, 'easeOutCubic');
        $text.transform({ x: -window.innerWidth }).tween({ x: 0 }, 500, 'easeOutQuint', 300);
    }
    
    this.fade = function(callback) {
        $overlay.visible().css({ opacity: 0 }).tween({ opacity: 0.5 }, 300, 'easeOutSine', function(){
