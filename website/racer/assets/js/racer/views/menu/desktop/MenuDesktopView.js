Class(function MenuDesktopView() {
    ...
    function initViews() {
        $logo = $('.logo');
        $logo.size(1000,275).mouseEnabled(false).css({ left: -463, top: -30, opacity: 0 }).bg(Config.PATH+'assets/images/common/logos/racer-large.png').setZ(1);
        $this.addChild($logo);
        
        _menu = _this.initClass(MenuDesktopViewMenu);
        _chrome = _this.initClass(ChromeExperimentView);
        _chrome.css({ zIndex: 100000 });
        __body.addChild(_chrome);
        _links = _this.initClass(LinksView);
        _links.css({ zIndex: 100000 });
    ...
    function keyPress(e) {
        ...
        if (c == 40 && _active !== -1 && _active < 4) menuClick({ index: _active+1 });
    }
    ...
    this.animateIn = function() {
        ...
        if (!_this.visible) {
            _this.visible = true;
            _chrome.animateIn();
            $logo.transform({ y: 10, x: -40 }).css({ opacity: 0 }).tween({ opacity: 1, y: 0, x: 0 }, 500, 'easeOutCubic', 400, function(){
                $logo.clearTransform();
                $logo.clearAlpha();
            });
