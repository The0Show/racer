Class(function MenuMobileView() {
  ...
  function initButtons() {
    ...
    $image
      .size("100%", 200)
      .css({ position: "relative", margin: "0 auto", backgroundSize: "cover" })
  ...
  function initError() {
    ...
    switch (Global.BAD_PERFORMANCE) {
      case "latency":
        $errorText.text(
          "It looks like<br/>your connection is too slow.<br/>Wifi works best."
        );
        break;
      case "framerate":
    ...
    _errorButton = _this.initClass(HorizontalButtonView, {
      text: "LEARN MORE",
    });
  ...
  this.animateIn = function () {
    ...
    if ($text)
      $text
        ...
        .css({ opacity: 0 })
        .transform({ y: 5 })
        .tween({ opacity: 1, y: 0 }, 300, "easeOutCubic", delay + 500);
    ...
    if (_start) {
      ...
      $errorTitle
        .css({ opacity: 0 })
        .tween({ opacity: 1 }, 300, "easeOutSine", delay + 600);
      $errorText
        .css({ opacity: 0 })
        .tween({ opacity: 1 }, 300, "easeOutSine", delay + 800);
